...
interface LoginPageProps {
  onLoginSuccess: (username: string) => void; // Expect username on success
...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onNavigateToHome }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (username.trim() && password.trim()) { // Check if both fields are non-empty
      setError('');
      onLoginSuccess(username.trim()); // Pass the entered username
    } else {
